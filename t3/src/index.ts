...
function promptForBook(): Book {
  const bookTitle: string = (prompt("Enter the book title: ") ?? "");
  const bookAuthor: string = (prompt("Enter the book author: ") ?? "");
  const bookPublicationYear:number = +(prompt("Enter the book publication year: ") ?? 0);
